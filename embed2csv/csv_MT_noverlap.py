...
OUTPUT_CSV = "embeddings_csv/embeddings_MT_noverlap.csv"
...
for (label, audio_id), chunks in audio_chunks.items():
    ...
    embeddings = [e[1] for e in chunks]  # e[1] es lista de strings

    i = 0
    while i < len(embeddings):
        group = embeddings[i:i+3]
        if len(group) < 3:
            group += [group[-1]] * (3 - len(group))  # padding final

        concatenated = sum(group, [])  # concatena listas de str
        row_id = f"{audio_id}_em{i // 3}"
        row = [row_id, label] + concatenated
        rows.append(row)
        i += 3  # avanzar sin solapamiento
